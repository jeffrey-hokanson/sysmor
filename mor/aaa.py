...
class AAARationalFit(RationalFit):
	...
	The AAA constructs a partial-interpolant based on 
	...
	def _fit(self, lam0 = None):
		...
		residual = np.copy(self.h)
		self.Ihat = np.zeros(len(self.h), dtype = np.bool)
		for k in range(self.m+1):
			...
			# Update the residual
			residual = self.h - self.__call__(self.z)	
	...
	def _build_loewner(self):
		...
		L = (C.T * self.h[~self.Ihat]).T - C*self.h[self.Ihat]
		return L

	def __call__(self, zeval):
		zhat = self.z[self.Ihat]
		hhat = self.h[self.Ihat]
