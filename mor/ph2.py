...
#				lam_can = lam[ (active_mask[::2] + active_mask[1::2]) == 0]
			...
			#LSsignificance = np.zeros(max_angles.shape)
			for i in range(len(lam)):
				# determine the norm of each component 
				#Hpole = PoleResidueSystem(lam[i], rho[i])
				#significance[i] = Hpole.norm()/Hr_norm
				#Hpole_mu = Hpole.transfer(mu).reshape(n,)
				#LSsignificance[i] = np.linalg.norm(M(Hpole_mu))/np.linalg.norm(M(H_mu))
				max_angles[i] = np.max(subspace_angle_V_M(mu, lam[i], L = L, d = d, p = p))
				#print "significance: %5.2e ; angle %12.8f" % (LSsignificance[i], 180.*max_angles[i]/np.pi), lam[i], np.abs(rho[i])
				#print "angle %12.8f" % (180.*max_angles[i]/np.pi,), lam[i], np.abs(rho[i])
				#if LSsignificance[i]< 1e-4:
				#	max_angles[i] = -1
			...
			if self.verbose:
				# Header
				if it == 0:
					print("  it | dim | FOM Evals | delta Hr |  cond M  |       mu star      | res norm | max angle |")
					print("-----|-----|-----------|----------|----------|--------------------|----------|-----------|")
				print("%4d | %3d |   %7d | %8.2e | %8.2e | %8.2e%+8.2ei | %8.2e | %9.6f | %8.2e" % 
					(it,rom_dim, self._total_fom_evals, delta_Hr, cond_M, mu_star.real, mu_star.imag,
					residual_norm, 180*max_angle/np.pi, (H-Hr).norm()/H.norm() ) )
...
if __name__ == '__main__':
	...
	H = H[0,1]
	Hr = ProjectedH2MOR(18, maxiter = 100, verbose = True, cond_max = 1e15, ftol = 1e-7)
	Hr.fit(H)	
